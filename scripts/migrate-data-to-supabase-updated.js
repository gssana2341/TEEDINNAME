...
// คำสั่งรัน: node scripts/migrate-data-to-supabase-updated.js
...
// ID ของเอเจนต์ (ในระบบจริง คุณจะใช้ ID จริงของเอเจนต์)
...
async function migrateData() {
  ...
  try {
    // สร้าง bucket สำหรับเก็บรูปภาพ (ถ้ายังไม่มี)
    ...
    if (bucketError && bucketError.message !== 'Duplicate name') {
      console.error('เกิดข้อผิดพลาดในการสร้าง bucket:', bucketError);
    } else {
      console.log('สร้าง bucket "property-images" สำเร็จ หรือมีอยู่แล้ว');
    }
  } catch (error) {
    console.log('อาจมี bucket อยู่แล้ว หรือเกิดข้อผิดพลาด:', error.message);
  }
