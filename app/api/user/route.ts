...
export async function GET(request: NextRequest) {  try {
    ...
    const { data: authData, error: authError } = await supabaseWithToken.auth.getUser()
    ...
    const { data: userData, error: userError } = await supabase
      .from('users')
      .select('id, email, role, first_name, last_name, phone, password, created_at, updated_at')
      .eq('email', userEmail)
      .limit(1)
      .single()
  ...
  try {
    ...
    if (password && password !== '••••••••') {
      ...
      if (authUpdateError) {
        ...
        console.log('⚠️ Auth password update failed but custom table updated successfully')
      } else {
  ...
  try {
    ...
    if (authUser) {
      ...
      if (authDeleteError) {
        ...
        console.log('⚠️ ไม่สามารถลบจาก Supabase Auth ได้ แต่ลบจาก users table สำเร็จแล้ว')
      } else {
