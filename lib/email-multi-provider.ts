...
const sendGmailSMTP = async (to: string, otpCode: string): Promise<{ success: boolean; error?: string }> => {
  try {
    console.log('📧 กำลังส่งอีเมล OTP ผ่าน Gmail SMTP...')
    
    if (!process.env.SMTP_USER || !process.env.SMTP_PASS || process.env.SMTP_PASS === 'your-16-digit-app-password-here') {
...
export const sendOTPEmail = async (
  ...
  if (process.env.SENDGRID_API_KEY && process.env.SENDGRID_API_KEY !== 'SG.demo_key_will_be_replaced') {
