...
© 2025 TedIn Easy - แพลตฟอร์มอสังหาริมทรัพย์ออนไลน์
    ...
    console.error('❌ เกิดข้อผิดพลาดในการส่งอีเมล Resend:', error);
    return {
      success: false,
      error: `ไม่สามารถส่งอีเมล OTP ได้: ${error.message}`,
      provider: 'resend'
    };
  }
}
...
export async function testResendConnection() {
  try {
    ...
    return {
      success: false,
      message: `ไม่สามารถเชื่อมต่อ Resend API ได้: ${error.message}`,
      error: error
    };
  }
}
